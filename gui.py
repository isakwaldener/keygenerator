...
class gui():
    ...
    def drawBoard(self):
        ...
        # nextChar = chr(self.game.keygenerator.getKeys())
        chars = [chr(i) for i in self.game.keygenerator.getKeys()]

        # text = font.render(f"{char} {nextChar:>5}", 1, (0, 0, 0))
...
gui = gui()
gui.game.setModes()
# gui.game.setCurrentMode("allKeys") # got some unknown keys
count = 0
exit = False
while(not exit):
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == gui.game.getKey(0):
                gui.game.removeKey()
